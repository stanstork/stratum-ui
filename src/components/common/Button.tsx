...
const Button: React.FC<ButtonProps> = ({ children, onClick, variant = 'primary', className = '', disabled = false, title = '', size = 'default' }) => {
    const baseClasses = 'rounded-lg font-semibold text-sm inline-flex items-center justify-center transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 transform active:scale-95';
    const sizeClasses = {
        default: 'px-4 py-2',
    ...
    return <button onClick={onClick} disabled={disabled} className={`${baseClasses} ${sizeClasses[size]} ${variants[variant]} ${className} ${disabled ? 'opacity-60 cursor-not-allowed' : ''}`} title={title}>{children}</button>;
};
