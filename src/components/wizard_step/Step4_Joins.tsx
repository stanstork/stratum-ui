...
const JoinItem: React.FC<JoinItemProps> = ({
    ...
    return (
        <div className="p-5 rounded-lg bg-white dark:bg-slate-800/80 border border-slate-200/80 dark:border-slate-700/80">
            ...
            <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isSchemaVisible ? 'max-h-[40rem] pt-4' : 'max-h-0'}`}>
                {joinTableSchema ? (
    ...
    return (
        <>
            ...
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-2xl font-bold text-slate-800 dark:text-slate-100">Table Joins</h2>
                    <p className="text-slate-500 dark:text-slate-400 mt-1">
                        Combine data from other tables with your source table.
                    </p>
                </div>
                <div className="flex justify-end gap-2">
                    <Button onClick={addJoin} variant="outline" disabled={allTablesInConnection.filter(t => !new Set(loadStep.entities.concat(sourceTableName)).has(t.name)).length === 0}>
