...
const isFunctionCall = (expr: Expression): expr is FunctionCallExpr =>
    Array.isArray((expr as FunctionCallExpr)?.FunctionCall);
...
const getEmptyExprForMode = (mode: SourceMode): Expression => {
    switch (mode) {
        ...
        case "arithmetic":
            return {
                Arithmetic: {
                    left: { Lookup: { entity: "", field: null, key: "" } },
                    operator: "Add",
            ...
            <div className="flex-grow">
                {isArgLookup ? (
                    <ColumnSelector
                        ...
                        onColumnChange={(val) =>
                            onUpdate({ Lookup: { entity: (arg as LookupExpr).Lookup?.entity || "", key: val, field: "" } })
                        }
                    />
                ) : (
                    <Input
    ...
    return (
        <div className="flex flex-col gap-2">
            <Input
                ...
                placeholder="Function Name (e.g. CONCAT)"
            ...
            <div className="pl-4 border-l-2 border-slate-200 dark:border-slate-700 space-y-2 mt-2 pt-2">
                ...
                {args.map((arg, i) => (
                    <div key={i} className="flex items-center gap-2">
                        <FunctionArgumentEditor arg={arg} onUpdate={(newArg) => handleArgChange(i, newArg)} allAvailableTables={allAvailableTables} />
                        <button
                            type="button"
                            onClick={() => removeArg(i)}
                ...
                <Button type="button" onClick={addArg} variant="outline" className="w-full">
                    <Plus size={14} className="mr-2" /> Add Argument
                </Button>
            </div>
        </div>
    );
    ...
    return (
        <div className="space-y-3">
            <ColumnSelector
                ...
                onColumnChange={(val) =>
                    handleUpdate("left", { Lookup: { entity: (left as LookupExpr).Lookup?.entity, key: val, field: "" } })
                }
            />
...
const MappingRow: React.FC<MappingRowProps> = ({
    ...
    return (
        <Card className="bg-white dark:bg-slate-800/50 rounded-xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm overflow-hidden">
            <CardContent className="p-5 grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
                {/* Source */}
                <div className="md:col-span-7">
                    ...
                    <div className="min-h-[60px]">
                        {mode === "lookup" && (
                            <ColumnSelector
                                allTables={allAvailableTables}
                                selectedTable={(mapping.source as LookupExpr).Lookup?.entity || ""}
                                selectedColumn={(mapping.source as LookupExpr).Lookup?.key || ""}
                                onTableChange={(val) => onUpdate(index, "source", { Lookup: { entity: val, key: null, field: "" } })}
                                onColumnChange={(val) =>
                                    onUpdate(index, "source", { Lookup: { entity: (mapping.source as LookupExpr).Lookup?.entity, key: val, field: "" } })
                                }
                            />
                        )}
                        {mode === "literal" && (
                            <LiteralEditor
                                ...
                                onUpdate={(newExpr) => onUpdate(index, "source", newExpr)}
                ...
                <div className="md:col-span-1 flex items-center justify-start md:justify-end h-full">
                    <button
                        ...
                        className="p-2 text-slate-400 hover:text-red-600 dark:hover:text-red-400 transition-colors rounded-md"
                    >
                        <X size={18} />
                    </button>
                </div>
...
const AvailableColumnsPanel: React.FC<AvailableColumnsPanelProps> = ({
    ...
    if (isCollapsed) {
        return (
            <Card className="bg-white dark:bg-slate-800/50 rounded-xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm overflow-hidden h-fit">
                <CardContent className="p-2 flex justify-center">
                    <Button type="button" onClick={onToggle} variant="ghost" title="Show Available Columns">
                        <ChevronsRight />
                    </Button>
                </CardContent>
            </Card>
        ...
        <Card className="bg-white dark:bg-slate-800/50 rounded-xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm overflow-hidden h-fit">
            <CardContent className="p-4 space-y-3">
                <div className="flex justify-between items-center">
                    ...
                    <Button type="button" onClick={onToggle} variant="ghost" title="Hide Panel">
                        <ChevronsLeft />
                    </Button>
                </div>
                ...
                <div className="space-y-1 max-h-[60vh] overflow-y-auto">
                    {columns.map(({ table, column }) => {
                        ...
                        return (
                            <div
                                key={`${table}.${column}`}
                                className={`flex items-center justify-between rounded-md ${isMapped ? "bg-slate-100 dark:bg-slate-700/50 p-2" : ""}`}
                            >
                                ...
                                {isMapped ? (
                                    <span className="text-green-500 flex items-center text-xs gap-1 font-medium">
                                        <Check size={14} /> Mapped
                                    </span>
                                ) : (
                                    <Button type="button" variant="ghost" onClick={() => onAddMapping(table, column)}>
                                        Add
                                    </Button>
                                )}
                            </div>
            ...
            {(allAvailableTables) => {
                ...
                const filteredColumns = !columnSearchTerm
                    ...
                    : allSourceColumns.filter(
                        (c) =>
                            c.column.toLowerCase().includes(columnSearchTerm.toLowerCase()) ||
                ...
                return (
                    <>
                        ...
                        <div className="flex justify-between items-center">
                            <div>
                                <h2 className="text-xl font-semibold text-slate-900 dark:text-white">Column Mapping</h2>
                                <p className="text-sm text-slate-600 dark:text-slate-300 mt-1">
                                    Define the structure of your destination table by mapping source columns and expressions.
                                </p>
                            </div>
                            <div className="flex justify-end gap-2">
                                <Button type="button" onClick={() => autoMapAll(allAvailableTables)} variant="outline">
                                    <Wand2 size={16} className="mr-2" /> Auto-map 1:1
                                </Button>
                                <Button type="button" onClick={() => addMapping()} variant="outline">
                                    ...
                                    {mappings.length === 0 ? (
                                        <Card className="bg-white dark:bg-slate-800/50 rounded-xl border border-slate-200/80 dark:border-slate-700/80 shadow-sm overflow-hidden">
                                            <CardContent className="py-16 text-center">
                                                ...
                                                <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-200">No Mappings Defined</h3>
